...
def plot_four_results(csv_a, csv_b, csv_c, csv_d, save_path=None):
    """
    Plots training loss, validation loss, and validation accuracy
    from exactly four CSV files.

    각 CSV 파일은 'epoch', 'train_loss', 'val_loss', 'val_accuracy' 열을 포함해야 함.
    x축은 epoch이 아니라, 모든 러닝이 0~1로 정규화된 구간으로 표시됨.
    """
    csv_files = [csv_a, csv_b, csv_c, csv_d]
    ...
    fig.suptitle("Comparison of Training Metrics (4 runs, normalized x-axis)", fontsize=20)

    # 공통 그리기 함수
    def plot_metric(ax, col, title, ylabel):
        for label, df in zip(labels, dataframes):
            x = np.linspace(0, 1, len(df[col]))  # 0~1 정규화된 구간
