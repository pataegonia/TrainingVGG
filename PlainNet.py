...
# 3. Plain 34-Layer Network Definition
# This block is the "plain" counterpart to the BasicBlock, without the residual connection.
class PlainBlock(nn.Module):
    """
    A plain convolutional block, counterpart to the ResNet BasicBlock but without
    the shortcut connection.
    """
    # NOTE: expansion is kept for compatibility with the ResNet builder class,
    # but it is always 1 for this block type.
    expansion = 1
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # --- The shortcut connection is removed from this block ---

    def forward(self, x):
        # --- The identity is not saved or added back ---
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        out = self.conv2(out)
        out = self.bn2(out)
        # --- out += identity is removed ---
        out = self.relu(out)
        return out
...
class PlainNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        # Initial convolution layer (stem), identical to ResNet
        ...
        # The four main layers, constructed with PlainBlocks
    ...
    def _make_layer(self, block, out_channels, num_blocks, stride):
        ...
        layers = []
        for s in strides:
            # Note: For plain networks, self.in_channels does not equal out_channels * block.expansion
            # on the first block of a new layer. This is handled naturally by the PlainBlock definition.
...
def PlainNet34(num_classes=10):
    """
    Constructs a 34-layer plain network.
    """
    return PlainNet(PlainBlock, [2, 2, 2, 2], num_classes)

# 4. Initialize model, loss function, and optimizer as per the paper
model = PlainNet34(num_classes=NUM_CLASSES).to(DEVICE)
    ...
    with open(RESULTS_FILE, 'w', newline='') as csvfile:
        ...
        for epoch in range(EPOCHS):
            ...
            if accuracy > best_accuracy:
                ...
                model_path = os.path.join(MODEL_SAVE_DIR, f'best_model_plainnet34.pth')
