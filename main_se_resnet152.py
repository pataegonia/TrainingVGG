...
# 3. Squeeze-and-Excitation Block Definition
...
# 4. SE-ResNet-152 Model Definition
# This class is structured to follow the ResNet architecture with SE blocks integrated.
class SEBottleneck(nn.Module):
    """
    The SE-bottleneck residual block for SE-ResNet-152.
    It uses a 3-layer shortcut connection with SE block integrated.
    """
    expansion = 4
    ...
    def __init__(self, in_channels, out_channels, stride=1, reduction=16):
        ...
        # 1x1 convolution to restore and expand channels
        self.conv3 = nn.Conv2d(out_channels, self.expansion * out_channels, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(self.expansion * out_channels)
        self.relu = nn.ReLU(inplace=True)
        
        # SE Block - applied after the third convolution
    ...
    def forward(self, x):
        ...
        # Apply SE block before adding residual connection
        out = self.se(out)
...
class SEResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        # The four main layers of the SE-ResNet architecture
...
def SEResNet152(num_classes=10):
    # For SE-ResNet-152, we use the SEBottleneck block with the configuration [3, 8, 36, 3]
    # This is the deepest standard ResNet variant with significantly more blocks in layer2 and layer3
    return SEResNet(SEBottleneck, [3, 8, 36, 3], num_classes)

# 5. Initialize model, loss function, and optimizer as per the paper
model = SEResNet152(num_classes=NUM_CLASSES).to(DEVICE)
    ...
    with open(RESULTS_FILE, 'w', newline='') as csvfile:
        ...
        for epoch in range(EPOCHS):
            ...
            if accuracy > best_accuracy:
                ...
                model_path = os.path.join(MODEL_SAVE_DIR, f'best_model_se_resnet152.pth')
...
if __name__ == '__main__':
    train_model()
    print("Training and validation finished! Results saved to SENet152.csv.")